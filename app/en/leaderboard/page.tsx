      ...
      <Section className="relative border-b border-white/8 bg-[#111317] pt-16 pb-8">
        <Container>
          <div className="max-w-3xl mx-auto text-center">
            <p className="font-[family-name:var(--font-space-grotesk)] text-xs uppercase tracking-widest text-[#F2CA50] font-bold mb-3">Top Builds</p>
            <h1 className="font-[family-name:var(--font-anton)] text-4xl md:text-6xl text-white uppercase tracking-wide mb-6">Leaderboard</h1>
            <p className="text-[#A8A8B3] text-lg leading-relaxed">The highest-rated Hoopers created by the community. Climb the board by building smart and simulating strong.</p>
          </div>
        </Container>
      </Section>
      ...
      <Section>
        <Container>
          <div className="max-w-4xl mx-auto">
            {loading ? (
              <div className="text-center py-12 text-[#A8A8B3]">Loading leaderboard...</div>
            ) : hoopers.length === 0 ? (
              <div className="text-center py-12 text-[#A8A8B3]">No builds yet. Be the first to create one.</div>
            ) : (
...
function LeaderboardRow({ hooper, rank }: { hooper: HooperEntry; rank: number }) {
  ...
  return (
    <a
      href={`/en/hooper?slug=${hooper.slug}`}
